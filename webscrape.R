...
x <- LinkExtractor('https://www.tcdb.com/ViewAll.cfm/sp/Baseball?MODE=Years' , 
                   urlregexfilter = 'year')
...
types <- setlist %>% select(type) %>% distinct() %>% 
  mutate(league = if_else(!type %in% c('College' , 'Draft' , 'Homemade' , 'Japanese League' , 
                                       'Korean League' , 'Minor League' , 'Unlicensed') , 
                          'Major League Baseball' , type) , 
         league = if_else(type %in% c('Draft' , 'Homemade' , 'Unlicensed') ,
                          'Other' , league))
